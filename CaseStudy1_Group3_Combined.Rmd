...
orderedMedian <- medianData %>% arrange(desc(ABV))

g.mid <- ggplot(orderedMedian,aes(x=1,y=State)) +
...
g1 <- ggplot(data = orderedMedian, aes(x = State, y = ABV, label = ABV)) +
...
g2 <- ggplot(data = orderedMedian, aes(x = State, y = IBU, fill = IBU, label = IBU)) +
  xlab(NULL) +
